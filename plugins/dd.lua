...
local XCommands = 
{
    LockCommand = "warn", -- The command to lock the link sharing
    UnlockCommand = "lnkkkkkkkk", -- The command to unlock the link sharing
    CheckCommand = "?" -- The command to check for the statue of the link sharing
}

local msgs = 
{
    already_locked = "تحذير ⚠️الروابط🚩 مفعل ✅اصلا", -- the message that sent when you try to lock the sharing and it's already locked
    Locked = 'تم ✅تفعيل تحذير⚠️ الروابط🔰', -- the message that send when you lock the sharing

    already_unlocked = "تحذير ⚠️الروابط🚩 مامفعل ❌اصلا", -- the message that sent when you try to unlock the sharing and it's already unlocked
    UnLocked = "تم✅ فتح 🔓تحذير⏪ الروابط", -- the message that send when you unlock the sharing
