...
local function run(msg, matches) 
  local jalal = msg['id'] 
  if matches[1] == 'media warn' then
    if is_momod(msg) then 
      local hash = 'mate:'..msg.to.id 
      redis:set(hash, true) 
      local x = ' تم تفعيل خاصية منع الميديا #بلتحذير. ❗️⛔️'
      reply_msg(jalal, x, ok_cb, true) 
    else 
      local asdy = 'هاذ الامر للمشرفين #فقط❗️' 
      reply_msg(jalal, asdy, ok_cb, true) 
    end
  end
  if matches[1] == 'media off' then
    if is_momod(msg) then 
      local hash = 'mate:'..msg.to.id 
      redis:del(hash) 
      local don = ' تم الغاء تفعيل خاصية منع الميديا #بلتحذير. ❗️⛔️' 
      reply_msg(jalal, don, ok_cb, true) 
    else
      local jalal_aldon = 'هاذ الامر للمشرفين #فقط❗️' 
